...
1. **频率限制 (基于 IP)**：每个 IP 每 60 秒限查询/解绑 5 次，超出需等待冷却。
2. **设备数限制 (基于卡密)**：每个卡密每月累计允许自助解绑 10 次，防止账号非法共享。
3. **设备名称脱敏**：列表中的设备名称自动打星号处理（如：`My-iPhone` → `My-*****`）。
4. **状态校验**：仅 `active` 状态卡密可进入门户，`revoked` (已吊销) 自动拦截。
