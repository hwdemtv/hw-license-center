...
### 📡 互联与动态控制
- **大模型 BFF 网关 (AI Proxy)**：内置兼容 OpenAI SDK 的代理网关，凭卡密 JWT 即可无感调用 AI（如智谱 GLM-4-Flash），有效隐藏真实 API Key 并支持细粒度的单卡每日调用配额管束。
- **Webhook 发卡全动态化**：后台支持可视化配置 Webhook 密钥，实时热更新生效，完美适配第三方个人发卡网或聚合支付网关。
- **配置热拔插中心**：彻底告别 `.env` 重启魔咒。从解绑额度、AI 设置、门户页面标题，均可在「系统设置」面板秒级在线调整。
- **全域精准广播 (v23+)**：内置广播通知系统。支持全局广播或基于产品 ID 的精准定向推送，`/verify` 接口毫秒级下发，完美适配版本更新与定向营销。
...
- **可视化看板 (v27+)**：管理后台内置纯 CSS 环形图与进度条，实时展示状态分布、设备占用、产品分布与订阅统计。
...
| **网关** | `POST /api/v1/ai/chat/completions` | AI 请求代理转发 (需带 Token) |
| **网关** | `GET /api/v1/ai/quota` | 查询卡密今日剩余 AI 调用额度 |
...
- [x] **Phase 27**: 看板可视化图表 (状态分布环形图、设备占用、产品分布、订阅状态统计)。
- [ ] **Phase 28 (近期)**: 自动化容灾备份至 S3/R2，补全 API 审计日志。
- [ ] **Phase 29 (展望)**: 多租户 SaaS 架构支持，更多图表类型 (趋势图、热力图)。
