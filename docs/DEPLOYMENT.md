# 部署方案指南 (v22+)

本文档说明如何将本卡密管理系统部署到不同环境中。本项目目前已实现 **双栖架构 (Dual Deployment)**，一套代码可原生运行于边缘计算与私有服务器。

---

## 📋 目录

- [方案一：Cloudflare Workers（极简部署）](#方案一cloudflare-workers极简部署)
- [方案二：私有 VPS / Node.js 部署（独立掌控）](#方案二私有-vps--nodejs-部署独立掌控)
- [部署对比](#部署对比)
- [常见问题](#常见问题)

---

## 方案一：Cloudflare Workers（极简部署）

### 架构
```
Cloudflare Workers (全球边缘节点)
    ↓
Cloudflare D1 Database (边缘 SQLite)
```

### 部署步骤

1. **准备环境**
   ```bash
   npm install
   wrangler login
   ```

2. **创建与初始化数据库**
   ```bash
   npx wrangler d1 create hw-db
   # 将输出的 database_id 填入 wrangler.toml
   npx wrangler d1 execute hw-db --remote --file=./schema.sql
   ```

3. **配置密钥**
   ```bash
   npx wrangler secret put ADMIN_SECRET
   npx wrangler secret put JWT_SECRET
   ```

4. **一键发射**
   ```bash
   npx wrangler deploy
   ```

---

## 方案二：私有 VPS / Node.js 部署（独立掌控）

### 架构优势
本项目底层通过 `DBAdapter` 屏蔽了存储差异。在 VPS 环境下，系统会自动启用 **SQLite WAL (Write-Ahead Logging)** 模式，并注入高性能 PRAGMA 配置（如 `Synchronous=NORMAL`），确保单机并发能力最大化。

### 部署步骤

1. **环境准备**
   确保已安装 **Node.js 18+**。

2. **项目初始化**
   ```bash
   git clone <repo-url>
   cd hw-license-center
   npm install
   ```

3. **配置环境变量**
   在根目录创建 `.env` 文件：
   ```env
   ADMIN_SECRET=你的管理后台密钥
   JWT_SECRET=你的设备校验密钥
   PORT=3000
   ```

4. **初始化本地数据库**
   使用内置脚本或手动创建：
   ```bash
   # 确保目录存在
   mkdir -p .wrangler/state/v3/d1/miniflare-D1DatabaseObject/
   # 写入表结构
   sqlite3 .wrangler/state/v3/d1/miniflare-D1DatabaseObject/db.sqlite < schema.sql
   ```

5. **构建并启动 (推荐使用 PM2)**
   ```bash
   # 编译
   npm run build:node
   
   # 启动
   npm install -g pm2
   pm2 start dist/server.js --name "km-center"
   ```

---

## 🚀 部署对比

| 维度 | Cloudflare Workers | 私有 VPS (Node.js) |
|-----|-------------------|-------------------|
| **部署难度** | ⭐ 极低 | ⭐⭐ 中等 |
| **数据库** | Cloudflare D1 (分布) | 本地 SQLite (高性能 WAL) |
| **费用** | 免费额度极高 | 需支付服务器月费 |
| **数据掌控** | 托管在 CF 边缘 | 100% 数据自主所有 |
| **扩展性** | 自动无限垂直扩展 | 受限于单机硬件性能 |

---

## 💡 常见问题

### Q: 如何将旧的 VPS 部署升级以支持大模型 AI 网关？
A: 如果您之前已经部署了旧版本，更新代码后除了 `npm install` 和 `npm run build:node`，您还需要为本地的 SQLite 数据库追加 AI 相关的字段：
```bash
# 登录服务器并进入数据库存放路径
sqlite3 .wrangler/state/v3/d1/miniflare-D1DatabaseObject/db.sqlite

# 然后在 SQLite 交互行中按顺序执行以下升级语句：
ALTER TABLE Licenses ADD COLUMN ai_daily_quota INTEGER DEFAULT NULL;
ALTER TABLE Licenses ADD COLUMN ai_used_today INTEGER DEFAULT 0;
ALTER TABLE Licenses ADD COLUMN ai_last_reset_date TEXT DEFAULT NULL;
ALTER TABLE Licenses ADD COLUMN ai_model_override TEXT DEFAULT NULL;
ALTER TABLE Licenses ADD COLUMN ai_key_override TEXT DEFAULT NULL;
ALTER TABLE Licenses ADD COLUMN ai_base_override TEXT DEFAULT NULL;
.quit
```
*新部署的用户直接执行 `sqlite3 ... < schema.sql` 即可，新版 SQL 内已包含全部结构及默认配置。*

### Q: 是否需要由于部署平台不同而修改代码？
A: **不需要**。本项目已完成 `DBAdapter` 的抽象重构，系统启动时会自动识别环境并挂载对应的数据库驱动。

### Q: 如何进行数据迁移？
A: 
- **从 CF 到 VPS**：执行 `npx wrangler d1 export <db> --remote --output=dump.sql`，然后在 VPS 上用 `sqlite3` 导入。
- **从 VPS 到 CF**：将 `.db` 文件转换为 SQL 后执行 `wrangler d1 execute`。

---

## 总结
- **追求极致简单与免运维**：选 Cloudflare Workers。
- **追求数据绝对私密或国内访问提速**：选私有 VPS。
