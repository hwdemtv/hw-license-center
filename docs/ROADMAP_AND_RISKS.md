...
*   **[已完成] 多维度统计报表**：管理后台已支持可视化图表，包括状态分布环形图、设备占用进度条、产品分布条形图、订阅状态统计卡片。

### 3.4 全域静默认证架构 (Zero-Plugin Auth Hub) ⭐
*   **[已完成] 跨插件授权共享模式**：现已通过 `hw-auth-client` 包正式实现。
    - **实装案例**：SmartMP 插件已集成 `hw-auth-client`，通过读取 `.obsidian/hw-auth.json` 实现多域名容灾与静默激活。
    - **核心逻辑**：统一的 `AuthService` 调度，消除了用户重复输入卡密的痛点。
